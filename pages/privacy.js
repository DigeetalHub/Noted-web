    ...
    <div className="">
      ...
      <main className="mt-[7rem] px-4">
        ...
        <div className="" onClick={() => setIsActive(!isActive)}>
          ...
          {isActive && (
            <p className="text-sm">
              This privacy policy (“Policy”) describes how Noted Media Group in
              Florida (“Company,” “we,” and “our”) processes, collects, uses and
              shares personal data when using this website{" "}
              <Link
                className="border-b-brandPrimary500 border-b-2 text-blue-400"
                href="http://notedmediagroup.com"
              >
                http://notedmediagroup.com
              </Link>{" "}
              (the “Website”). Please read the following information carefully
        ...
        {data.map((item, index) => (
          <Accordion data={item} key={item.id} index={index} />
        ))}
