	...
	return (
		<div className="">
			...
			<main className="mt-[7rem] px-4">
				...
				{data.map(({ id, title, subtitle, description, paragraph, boldText, normalText, listItems }, index) => (
					<Accordion id={id} title={title} subtitle={subtitle} description={description} key={index} paragraph={paragraph} boldText={boldText} normalText={normalText} listItems={listItems} />
				))}
			</main>
		</div>
	);
